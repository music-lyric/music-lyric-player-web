<template>
  <div :class="$style.lyricArea">
    <div ref="container" :class="$style.lyricHost"></div>
    <div v-if="!player.hasLyric.value" :class="$style.lyricEmpty">
      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
        <path d="M9 18V5l12-2v13" />
        <circle cx="6" cy="18" r="3" />
        <circle cx="18" cy="16" r="3" />
      </svg>
      <p :class="$style.lyricEmptyTitle">{{ t('lyricArea.empty') }}</p>
      <p :class="$style.lyricEmptyHint">{{ t('lyricArea.emptyHint') }}</p>
    </div>
  </div>
</template>
